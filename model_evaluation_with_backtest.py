...
print("Classification Report:\n", classification_report(y_test_np, y_pred_labels))
print("Confusion Matrix:\n", confusion_matrix(y_test_np, y_pred_labels))

# Simple backtesting strategy
initial_balance = 10000  # Initial account balance in dollars
balance = initial_balance
position = 0  # Initial position (number of shares)
buy_price = 0  # Price at which the last buy occurred

for i in range(len(y_test_np)):
    if y_pred_labels[i] == 1 and balance > 0:  # Buy signal and have balance
        position = balance / X_test[i][0]  # Buy as many shares as possible with the available balance
        buy_price = X_test[i][0]  # Record the price at which shares were bought
        balance = 0  # Set balance to zero after buying

    elif y_pred_labels[i] == 2 and position > 0:  # Sell signal and have position
        balance = position * X_test[i][0]  # Sell all shares and calculate the balance
        position = 0  # Set position to zero after selling

# Calculate the final account balance after backtesting
final_balance = balance + position * X_test[-1][0]  # Account for any remaining position value
